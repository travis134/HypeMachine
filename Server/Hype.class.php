...
final class Hype extends Rating
{
	...
	public static function readAllWithValue($args)
	{
		...
		{
			$sth = $dbh->prepare("SELECT * FROM hype_table");
			$flag = $sth->execute();
		}
		else
		{
			$sth = $dbh->prepare("SELECT * FROM hype_table WHERE $arg_name = :$arg_name");
	...
	{
		$dbh = Db_Singleton::instance();
		$sth = $dbh->prepare("INSERT INTO hype_table (game_id, user_id, score) VALUES (:game_id, :user_id, :score)");
	...
	{
		$dbh = Db_Singleton::instance();
		$sth = $dbh->prepare("SELECT * FROM hype_table WHERE (id = :id) OR (game_id = :game_id AND user_id = :user_id) LIMIT 0, 1");
	...
	{
		$dbh = Db_Singleton::instance();
		$sth = $dbh->prepare("UPDATE hype_table SET game_id = :game_id, user_id = :user_id, score = :score WHERE id = :id");
	...
	{
		$dbh = Db_Singleton::instance();
		$sth = $dbh->prepare("DELETE FROM hype_table WHERE id = :id");
